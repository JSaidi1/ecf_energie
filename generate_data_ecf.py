...
def generate_consommations_test(
    input_path: str = OUTPUT_DIR / "consommations_raw.csv",
    output_path: str = OUTPUT_DIR / "consommations_raw-test.csv",
    n_lines: int = 1000
):
    """
    Genere le fichier consommations_raw-test-1000.csv de test qui est une copie
    du fichier consommations_raw.csv avec n_lines lignes.
    """
    ...
    with open(input_path, "r", encoding="utf-8") as infile, \
        ...
        # Copier seulement n_lines lignes
        for i, line in enumerate(infile):
            if i >= n_lines:
                break
            outfile.write(line)

    print(f"Generated {n_lines} consommations tests records -> {output_path}")
...
def main():
    ...
    # Generer les consommations de tests (1000 ligne seulement)
    generate_consommations_test()
    print()
